...
{
    unsigned long long int i, a_count = 0, o_count = 0;
    for (i = 0; i < apples.size(); i++)
    {
        unsigned long long int apple_dist = 0;
        apple_dist = a + apples[i];
        if (apple_dist >= s && apple_dist <= t) // && not ||
        {
            a_count++;
        }
    }
    ...
    {
        unsigned long long int orange_dist = 0;
        orange_dist = b + oranges[i];
